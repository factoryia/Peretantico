...
function parseOptions(rawOptions: unknown): string[] {
  ...
  if (typeof rawOptions === "object" && rawOptions !== null && "items" in rawOptions) {
    const items = (rawOptions as any).items;
    if (Array.isArray(items)) {
      return items.map((x: any) => x.value || x.label || String(x)).filter(Boolean);
    }
  }
  return [];
}
...
export const validateServiceField = createTool({
  ...
  args: jsonSchema<{ serviceId: string; fieldId: string; value: string }>({
    type: "object",
    properties: {
