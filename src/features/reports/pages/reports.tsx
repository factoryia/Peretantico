  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-6 bg-gray-50 rounded-md border">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {reportStats.map(({ title, value, Icon, valueClass, iconBackground }) => (
          <Card>
            <CardContent className="flex items-center justify-between px-6">
              <div>
                <p className="text-sm font-medium text-gray-500">{title}</p>
                <h3 className={cn("text-3xl font-bold mt-1", valueClass)}>{value}</h3>
              </div>
              <div className={cn("p-3 rounded-full", iconBackground)}>{Icon}</div>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <Card>
        <CardContent className="px-6">
          <h2 className="text-xl font-semibold mb-6 border-b pb-3">Generar Reportes</h2>
